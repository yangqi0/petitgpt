...
def _build_prompt(task: str, item: dict[str, Any]) -> tuple[str, str]:
    ...
    if task == "arithmetic":
        user = item["prompt"]
        return task, SYS_ARITH.rstrip() + "\n\n" + user.rstrip() + "\n\n"
    if task == "syllogism":
        user = item["prompt"]
        return task, SYS_SYLL.rstrip() + "\n\n" + user.rstrip() + "\n\n"
    if task == "code":
        if "prompt" in item:
            user = item["prompt"]
        else:
...
def _normalize_indent_mod4(lines: list[str]) -> list[str]:
    """Snap each non-empty line's leading spaces down to a multiple of 4 (0,4,8,...).
    Fixes common artifacts like 1 extra leading space causing IndentationError.
    Only removes spaces (never adds).
    """
    out: list[str] = []
    for ln in lines:
        if ln.strip() == "":
...
def run_python_tests(signature: str, body: str, tests: list[str]) -> tuple[bool, str]:
    ...
    body_lines = _normalize_indent_mod4(body_lines)
...
def main() -> None:
    ...
    for it in items:
        ...
        if task == "code":
            ...
            max_new = min(args.max_new_tokens, 8)
        else:
            stop_on_newline = True
            stop_string = None
            stop_regex = r"(?:yes|no|unknown)"
            restrict = "ynu"
            extra_ban = str(args.eos_id)
            max_new = min(args.max_new_tokens, 2)
