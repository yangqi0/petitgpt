...
# -*- coding: utf-8 -*-
"""
gen_sft_synthetic_v10_1.py

Single-turn synthetic SFT generator designed for:
  pretrain/train_sft_round1_exmean.py --loss_reduction example_mean

v10.1 changes vs v10:
- Make factorial more frequent in code.
- Add a 2nd factorial implementation (while-loop) to improve robustness.
- Still single-turn (no packs) to avoid "sequence completion" artifacts.
...
def _word_apples(rng: random.Random, hi: int = 40) -> Tuple[str, str, str]:
    ...
    b = rng.randint(0, hi)
    return (
        f"John has {a} apples and buys {b} more. How many apples does he have?\nAnswer:",
        str(a + b),
        "word",
    )
...
def gen_code(rng: random.Random, i: int) -> Dict:
    ...
    # v10.1: factorial higher
    weights = [0.14, 0.30, 0.24, 0.12, 0.20]
    ...
    if kind == "add":
        ...
        user = "Complete the following Python function:\n\ndef factorial(n):\n    "
        # two variants to improve robustness
        if rng.random() < 0.5:
            body = (
                "if n < 0:\n"
