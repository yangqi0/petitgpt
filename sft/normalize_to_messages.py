...
def parse_dolly(obj: dict[str, Any]) -> list[dict[str, str]]:
    """
    Parse Dolly-style instruction data into canonical messages.

    Common Dolly fields (varies by export/version):
    - instruction: str
    - context or input: str (optional)
    - response or output: str
    """
    instr = obj.get("instruction") or obj.get("prompt")
    ...
    instr = clean_text(str(instr))
    resp = clean_text(str(resp))

    if ctx and str(ctx).strip():
        ctx = clean_text(str(ctx))
        user = instr + "\n\n" + ctx
    else:
        user = instr
...
def main() -> None:
    ...
    ap.add_argument(
        "--source", required=True, choices=["ultrachat", "oasst", "dolly"], help="Dataset source type"
    )
    ap.add_argument(
