...
def compute_sft_loss(
    ...
    """
    logits: [B, T, V] already shifted (i.e., excludes last time step)
    labels: [B, T] next-token targets aligned to logits
    reduction:
      - token_mean: standard CE averaged over all supervised tokens in batch
      - example_mean: compute mean CE per-example (over supervised tokens), then average across batch
    Returns:
      loss scalar tensor
      stats dict
    """
    ...
    if reduction == "token_mean":
        ...
        stats = {
            "sup_tokens": float(sup_tokens),
            "sup_tokens_per_ex": float(sup_tokens / max(1, B)),
        }
        return loss, stats
    ...
    if reduction == "example_mean":
        ...
        mask_f = mask.float()
        tok_cnt = mask_f.sum(dim=1).clamp_min(1.0)  # [B]
        loss_ex = (loss_tok * mask_f).sum(dim=1) / tok_cnt  # [B]
        loss = loss_ex.mean()
        stats = {
            "sup_tokens": float(sup_tokens),
            "sup_tokens_per_ex": float(sup_tokens / max(1, B)),
            "sup_tokens_min": float(tok_cnt.min().item()),
            "sup_tokens_max": float(tok_cnt.max().item()),
        }
        return loss, stats
...
def main():
    ...
    ap.add_argument("--loss_reduction", choices=["token_mean", "example_mean"], default="token_mean", help="Loss averaging. token_mean=old behavior; example_mean=average loss per example then mean over batch (prevents long code examples dominating).")
        ...
        with torch.no_grad():
            for input_ids, labels, attn in val_dl:
                ...
                with torch.autocast(
                    ...
                    loss, _ = compute_sft_loss(logits, tgt, reduction=args.loss_reduction)
    ...
    while step < args.steps:
        ...
        with torch.autocast(
            ...
            # labels already store next-token targets at position i-1
            tgt = labels[:, :-1].contiguous()
            loss_unscaled, loss_stats = compute_sft_loss(logits, tgt, reduction=args.loss_reduction)
        ...
        if step % 50 == 0:
            ...
            print(
                f"[train] step={step} loss={(loss.item()*args.grad_accum):.4f} lr={args.lr} elapsed_s={elapsed:.1f} sup_tok/ex={loss_stats.get('sup_tokens_per_ex',0):.1f} red={args.loss_reduction}"
            )
