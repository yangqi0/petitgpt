...
def test_ineligible_row_is_filtered_before_ranking_and_replaced(tmp_path: Path, stage: str):
    ...
    filtered_dir, manifest = _build(tmp_path, f"filtered_{stage}", str(source), eligibility=eligibility)
...
def test_production_cli_refuses_to_run_without_an_eligibility_manifest(
    ...
    monkeypatch.setattr(
        "sys.argv",
        [
            "build_reference_val_shards.py", "reserve",
            "--source", f"{source}:4",
...
def test_escape_hatch_and_manifest_are_mutually_exclusive(
    ...
    source = _write_jsonl(tmp_path / "source.jsonl", [f"doc {i}." for i in range(20)])
    eligibility = _write_eligibility(
        tmp_path / "elig", {source: [0]}, total_rows={source: 20}
    )
    monkeypatch.setattr(
        "sys.argv",
        [
            ...
            "--source", f"{source}:4",
            "--out_dir", str(tmp_path / "out"),
            "--eligibility_manifest", str(eligibility),
...
def test_eligibility_manifest_must_cover_every_source(
    ...
    eligibility = _write_eligibility(
        tmp_path / "elig", {covered: [0]}, total_rows={covered: 20}
    )
    monkeypatch.setattr(
        "sys.argv",
        [
...
def test_frozen_selection_semantics_are_unchanged_by_the_fix(tmp_path: Path):
    ...
    elig = _write_eligibility(
        tmp_path / "elig", {source: []}, total_rows={source: len(texts)}
    )
    ...
    assert (
        (unfiltered_dir / EXCLUSION_MANIFEST_NAME).read_bytes()
        == (filtered_dir / EXCLUSION_MANIFEST_NAME).read_bytes()
    )
