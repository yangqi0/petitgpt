...
def test_21_duplicate_cleaned_identity_cannot_reach_the_output(tmp_path: Path):
    """Requirement 21. Global identity uniqueness is enforced at the writer, not assumed."""
    base = build_record(
        stage="stage_b",
        source_id="b_x",
    ...
    twin = dict(base)
    twin["stable_input_record_ordinal"] = 1
    from pretrain.stage_i_output_v1 import write_shards

    with pytest.raises(OutputError, match="duplicate cleaned identity"):
        write_shards(tmp_path / "stage", [base, twin])
...
def test_30b_manifest_reflects_the_realized_selection(tmp_path: Path, accepted_h):
    """The published manifest must restate the ledger, not a recomputation of it."""
    texts = [f"Document {i} about erosion." for i in range(6)]
    rows = [json.dumps({"text": t}) + "\n" for t in texts]
    ...
    manifest = build_manifest(graph, selections, accepted_h, environment)
    assert manifest["totals"]["records"] == sum(s.selected_identities for s in selections)
    assert manifest["totals"]["serialized_tokens"] == sum(
        s.selected_serialized_tokens for s in selections
    )
    assert manifest["nodes"][0]["selection_fingerprint"] == selections[0].selection_fingerprint
    assert manifest["h_binding"]["h_run_identity"] == accepted_h.run_identity


def test_30c_two_pass_spooling_emits_one_globally_ordered_stream(tmp_path: Path):
    """The spool path must produce the canonical physical order across nodes AND bindings.

    This exercises `iter_records_in_physical_order` rather than `materialize_binding` directly,
    because the spool-then-concatenate step is where a global ordering bug would live: each
    binding is read once, but its records can belong to several nodes.
    """
    from pretrain.stage_i_realize_v1 import iter_records_in_physical_order

    texts = [f"Document {i} regarding kilns, tides and sediment layers." for i in range(10)]
    rows = [json.dumps({"text": t}) + "\n" for t in texts]
    graph = _scan_graph(
        tmp_path,
        rows,
    ...
    streamed = list(
        iter_records_in_physical_order(graph, str(TOKENIZER), selections, tmp_path / "work")
    )
    keys = [record_sort_key(r) for r in streamed]
    assert keys == sorted(keys), "the spooled stream is not in canonical physical order"
    assert len({r["cleaned_text_sha256"] for r in streamed}) == len(streamed)
    assert len(streamed) == sum(s.selected_identities for s in selections)
    # Stage B must come first in the layout, and both nodes must be represented.
    assert [r["source_id"] for r in streamed][0] == "b_x"
    assert {r["source_id"] for r in streamed} == {"b_x", "a_x"}
    # Spool files are transient working state and must not survive the stream.
    assert list((tmp_path / "work").glob("spool-*")) == []


def test_30d_publishing_the_spooled_stream_round_trips(tmp_path: Path, accepted_h):
    ...
    # The published set must be exactly the committed selection, no more and no less.
    assert {r["cleaned_text_sha256"] for r in published} == {
        d.cleaned_sha256 for s in selections for d in s.selected
    }


def test_24d_publisher_reconciles_the_shard_count_it_produced(tmp_path: Path):
    """Regression: the publisher owns `totals.shards`, because only it knows the shard list.

    `build_manifest` cannot fill this in -- the shard count is a consequence of writing the
    records -- so it emits a placeholder. Before the fix, `publish_atomic` set `shards` but left
    `totals.shards` at that placeholder, and `validate_manifest` then rejected every realization
    whose true shard count differed from it. The single-shard fixtures happened to agree with the
    placeholder and hid the defect; a real run would have failed at publication.
    """
    manifest, records = _publishable(tmp_path)
    manifest["totals"]["shards"] = 0  # the placeholder build_manifest actually emits
    final = publish_atomic(tmp_path / "out", "run-x", manifest, records)
    loaded = load_published_realization(final)
    assert loaded["totals"]["shards"] == len(loaded["shards"]) == 1

    # The caller's manifest must not have been mutated in place by publication.
    assert manifest["totals"]["shards"] == 0
    assert manifest["shards"] == []


def test_24e_multi_shard_publication_splits_on_the_declared_boundary(tmp_path: Path):
    """More records than one shard holds must split at exactly RECORDS_PER_SHARD, whole records."""
    count = RECORDS_PER_SHARD + 3
    ...
    def gen():
        for i in range(count):
            yield _record(
                stable_input_record_ordinal=i,
                selection_ordinal_within_node=i,
                cleaned_text_sha256=hashlib.sha256(f"m-{i}".encode()).hexdigest(),
                raw_sha256=hashlib.sha256(f"mr-{i}".encode()).hexdigest(),
                input_record_sha256=hashlib.sha256(f"mw-{i}".encode()).hexdigest(),
                canonical_fingerprint=hashlib.sha256(f"mf-{i}".encode()).hexdigest(),
    ...
    assert loaded["shards"][1]["records"] == 3
    assert [s["name"] for s in loaded["shards"]] == [
